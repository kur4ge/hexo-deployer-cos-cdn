...
function loadLocalFiles(cfgs, publicDir, localFileMap, localImgsMap) {
    let disallowFiles = cfgs.disallowFiles || []
    disallowFiles = disallowFiles.map((s) => {    // 转化为正则规则
        return s.replace(/[-\/\\^$+?.()|[\]{}]/g, '\\$&')   // 替换正则关键字 除了 *
    }).map(s => s.replace(/\*/g, '.*?')).map(s => new RegExp(`^${s}$`))

    if (!cfgs.imageConfig) {
        //图片未单独配置bucket,获取 publicDir目录中的文件列表，将图片和文件统一加入到 localFileMap 中。
        getFiles(publicDir, (file) => {
            for (let r of disallowFiles) {
                if (r.test(file)) {
                    console.log(chalk.gray('跳过上传 ' + file));
                    return
                }
            }
